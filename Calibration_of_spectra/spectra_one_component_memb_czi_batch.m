...
% Semi-automatic selection (select ROI, then threhsold-based selection,
% threshold=thresholdfactor x maximum signal)
thresholdfactor=1/5; 
...
for i=1:size(files,1) % go through all acquired measurements/images
    ...
    mask=roipoly;
    meanframeroi=mask.*meanframe;
    threshold=thresholdfactor*max(max(meanframeroi)); % Definition of threshold: All pixels with signal of at least 1/5 of the maxium pixel intensity are kept
    mask(meanframeroi<threshold)=0;

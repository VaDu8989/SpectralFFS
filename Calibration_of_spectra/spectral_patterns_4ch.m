...
pathApp=uigetdir; % select directory in which extracted spectra from species 3 were saved
pathCh=uigetdir; % select directory in which extracted spectra from species 4 were saved
...
filesApp=dir([pathApp '/*flspectrum.txt']);
...
spectra_App=zeros(specchannelnumb,size(filesApp,1));
...
for i=1:size(filesApp,1)
    [namedata,remain]=strtok(filesApp(i).name,'.');
    namefile=filesApp(i).name;
    specdataApp=load([pathApp '/' namefile]);
    spectra_App(:,i)=specdataApp(:,2);
end
% load all spectra from species 4:
...
hold on
errorbar(spectralchannels,avgspectrum_App,stdspectraApp/2,stdspectraApp/2);
hold on
errorbar(spectralchannels,avgspectrum_Ch,stdspectraCh/2,stdspectraCh/2);
xlabel('Channel [nm]')
ylabel('Norm.emission')
legend('Spectrum mEGFP','Spectrum mEYFP','Spectrum mApple','Spectrum mCherry2')

% Generate output .txt file:
p_ij=[avgspectrum_GFP';avgspectrum_YFP';avgspectrum_App';avgspectrum_Ch'];
fidpij=fopen([path2 '\spectral_patterns_G_Y_A_Ch.txt'],'a'); % adjust filename if necessary
fprintf(fidpij,'%e\t %e\t %e\t %e\n',p_ij);
